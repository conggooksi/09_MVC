...
import javax.sql.*;//DataSource, 커넥션풀 용
import javax.naming.*;//커넥션풀 용
...
public class BoardDAO {
	...
	//--------------------------------------------------
	//생성자 <-- private 해서 외부에서 객체 생성 못하게 막아버림
	private BoardDAO(){}
	
	//싱글톤 객체 사용할 것임, 메모리 절약 효과가 있음
	private static BoardDAO dao=new BoardDAO();//객체 생성
	
	//jsp에서 dao 객체 얻기 <-- 생성이 아니라 얻기
	...
	public void insertBoard(BoardDTO dto){
		...
		try{
			...
			if(rs.next()){//글이 존재하면
				number=rs.getInt(1)+1;//최대 글번호+1 <--필드가 하나밖에 없으니깐 getInt에 (1)해준거
			}else{//첫번째 글쓰기 일때
				number=1;//ref=number
			}//else-end
			...
			//날짜
			pstmt.setInt(4, ref);////조심하세요 <- dto에서는 답글일지 원글일지 모르기때문에 앞에서 처리한 걸로 사용
		...
		try{
			...
			if(rs.next()){
				...
				do{
					...
					System.out.println("날짜 :"+rs.getString("regdate"));//문자열
					System.out.println("날짜 :"+rs.getDate("regdate"));//날짜타입-년월일
					System.out.println("날짜 :"+rs.getTimestamp("regdate"));//날짜타입-년월일  시분초
					
					dto.setReadcount(rs.getInt("readcount"));
					dto.setRef(rs.getInt("ref"));//글 그룹
					dto.setRe_step(rs.getInt("re_step"));//글순서
					dto.setRe_level(rs.getInt("re_level"));//글깊이
	...
	public int updateBoard(BoardDTO dto){
		...
		try{
			...
			if(rs.next()){
				...
				if(dbpw.equals(dto.getPw())){//암호가 일치하면 글 수정
	...
	public int deleteBoard(int num,String inpw){
		...
		try{
			...
			if(rs.next()){
				dbpw=rs.getString("pw");
				if(inpw.equals(dbpw)){//암호가 일치하면 글 삭제
