...
public class ControllerAction extends HttpServlet{
	...
	public void init(ServletConfig config) throws ServletException{
		...
		//Iterator 는 모든 유틸들을 받아낼 수 있다, CommandPro.properties에 있는 것중 = 앞에꺼를 키라고 표현하여 keySet()
		while(keyItor.hasNext()){//자료가 있는 동안 반복
 			
			String key=(String)keyItor.next();// /board/list.do
 			String className=pr.getProperty(key);// action.ListAction
	...
	private void reqPro(HttpServletRequest request,HttpServletResponse response) 
		...
		try{
			...
			if(uri.indexOf(request.getContextPath())==0){//프로젝트명이 Context
